...
export const checkEmail = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Failed to check email", error: error.message });
  }
};
...
export const checkNickname = async (req, res) => {
  try {
    ...
    logger.error("Check nickname error", { error: error.message, stack: error.stack });
    res.status(500).json({ message: "Failed to check nickname", error: error.message });
  }
};
